...
public class Bank {
	...
	public void Retrieve() {
		...
		in.nextLine();
		//AccountRecordSerializable pRead[];
		int count = 0;
		try {
			...
			while(isExist) {
				if(fis.available() != 0) {
					pRead[count] = (AccountRecordSerializable) in.readObject();
					count = count + 1;	
		...
		int i;
		pRead = new AccountRecordSerializable[count];
		for(i=0; i<count; i++) {
			int s = pRead[i].getAccount();
			if(s == aN) {
				System.out.println(pRead[i]);
				break;
			}
		}
		if(i == count) 
